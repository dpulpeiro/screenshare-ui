...
function Share() {
  ...
  const ws = useRef<WebSocket>(
    new WebSocket(`${process.env.NODE_ENV === 'development' ? 'ws' : 'wss'}://${process.env.REACT_APP_API}/ws/${uuid}`,
    ),
  )
  const url = useRef(`${window.location.protocol}//${window.location.host}/view/${uuid}`)
  const pc = useRef<any>()
